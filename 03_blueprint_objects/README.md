...
| Command | Result |
|---|---|
| `compose.py military-science-pack 1` | 9 intermediates, 10 lanes, 3 direct links, 59x84, 1,003 entities (`--no-links`: 13 lanes, 81x84, 1,191); IN iron-ore 5.75/s, copper-ore 0.5/s, coal 5/s, stone 10/s; OUT 1/s |
...
| `compose.py military-science-pack 10 --raw iron-plate --raw copper-plate --roboports` | 279x305, 10,077 entities, 42 roboports, one pole network |
| `compose.py military-science-pack 1 --roboports` | 73x86, 1,113 entities, 4 roboports (bottom-left first), all wired |
| `compose.py electronic-circuit 2 --raw iron-plate --raw copper-plate` | 19x14, 87 entities; plates fed from outside instead of smelted, cable linked straight into the circuits (`--no-links`: 24x32, 170 two passes ago) |
| `compose.py advanced-circuit 1 --from-plates` | 75x63, 1,083 entities; 11 lanes (5 pipe: crude, water, heavy, light, petgas); refinery + heavy/light cracking + plastic + cable + circuits; IN copper-plate 5/s, iron-plate 2/s, crude-oil 20.5/s, water 27.2/s, coal 1/s; OUT advanced-circuit 1/s |
| `compose.py advanced-circuit 1` | same from ore: 84x85, 1,287 entities |
...
| `compose.py utility-science-pack 1 --raw iron-plate --raw copper-plate --belt turbo-transport-belt` | 213x202, 5,237 entities, 22 modules, 20 lanes (6 pipe), 5 direct links, no warnings; 20 intermediates incl. flying robot frames, electric engine units, batteries, oil |
| `compose.py utility-science-pack 1 --from-plates` | same from plates on yellow belt: 413x177, 9,932 entities, 43 modules |
| `compose.py production-science-pack 1 --from-plates` | 282x217, 6,684 entities, 30 modules |
| `compose.py processing-unit 1 --from-plates --belt fast-transport-belt` | 195x110, 3,403 entities, 2 direct links; refinery + cracking, sulfur, sulfuric acid, plastic, cable, green and red circuits, processing units |
...
| North modules | `gap` apart, or **one column** apart when the next module is fed directly by the one before it, bottom-aligned on `by = max(north height)-1`. Sides are dealt on each side's east edge, not a cursor. North modules may sit over the riser wall — risers only occupy rows below the bus — as long as none of their chains runs down a column where a lane *above* them rises, or beside one (a lane ducking under the chain needs a free tile on each side). South modules share rows with the risers, so they start east of the wall; sides are dealt on progress from their own start, not on absolute column | `0..by` |
| North routing band | belt port k jogs on row `B0-1-curve0-k` (south: `spare+1+k`). `band_rows()` sizes it from the ports actually on that side: one row per jog index, **none at all** when every pull comes straight up its own port column, plus one more (`curve0`) only if a pull on lane 0 needs a splitter branch curve. The modules always keep one row above the first lane — the link row counts. `compose()` retries the whole search with one row of slack if a pull the candidate search had to move then has nowhere to jog | `by+1 .. by+R` |
...
| Factory | two passes ago | now | lanes | links |
|---|---|---|---|---|
| `circuits copper-cable=6 electronic-circuit=2` | 24x32 = 768, 170 entities | 19x14 = 266, 87 entities | 4 -> 3 | 1 |
| `electronic-circuit 2 --raw iron-plate --raw copper-plate` | 24x32 = 768, 170 entities | 19x14 = 266, 87 entities | 4 -> 3 | 1 |
| `plastics plastic-bar=2` | 15x13 = 195, 61 entities | 13x9 = 117, 40 entities | 3 | 0 |
| `military-science-pack 1` | 86x89 = 7,654, 1,280 entities | 59x84 = 4,956, 1,003 entities | 13 -> 10 | 3 |
| `advanced-circuit 1 --from-plates` | 92x68 = 6,256, 1,309 entities | 75x63 = 4,725, 1,083 entities | 12 -> 11 | 1 |
| `processing-unit 1 --from-plates --belt fast-transport-belt` | 198x140 = 27,720, 3,843 | 195x110 = 21,450, 3,403 | 18 -> 16 | 2 |
| `utility-science-pack 1 --raw ... --belt turbo-transport-belt` | 240x208 = 49,920, 5,842 | 213x202 = 43,026, 5,237 | 25 -> 20 | 5 |
| `production-science-pack 1 --from-plates` | 291x221 = 64,311, 7,328 | 282x217 = 61,194, 6,684 | 27 -> 26 | 1 |
| `military-science-pack 10 --raw ... --roboports` | 267x308, 10,063 | 279x305, 10,077 | 27 | 0 |
| `processing-unit 10 --raw ... --belt fast-transport-belt` | 1,095x320, 57,791 | 1,097x316, 56,469 | 71 | 0 |
...
| Name | Specs | Result |
|---|---|---|
| `circuits` | `copper-cable=6 electronic-circuit=2` | 3 lanes + 1 direct link, 19x14, IN copper-plate 3/s + iron-plate 2/s, OUT electronic-circuit 2/s |
| `inserters` | `inserter=1 electronic-circuit=1 iron-gear-wheel=1 copper-cable=3` | 4 lanes, 31x18, 3-input module pulling from 3 lanes |
| `nested` | `circuits.module.json iron-gear-wheel=1 inserter=1` | composite inside composite, 34x29 |
| `plastics` | `plastic-bar=2` | 13x9, IN petroleum-gas (pipe) 20/s + coal 1/s, OUT plastic-bar 2/s |
| `chips` | `sulfuric-acid=10 processing-unit=0.2` | 25x20, a chemical plant feeding an assembler by pipe: IN water 20/s (pipe), iron-plate 0.2/s, sulfur 1/s, electronic-circuit 4/s, advanced-circuit 0.4/s; OUT processing-unit 0.2/s |
